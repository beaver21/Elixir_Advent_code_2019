defmodule Day2 do
  ...
  def read_password(password) do
    [range, letter, password] = String.split(password)
  ...
  def find_letter_count(%{:letter => letter, :password => password}) do
    String.split(password, "")
    |> Enum.filter(fn l -> l == letter end)
    |> Enum.count()
  end

  def letter_count_is_between(password_read = %{:first_param => first_param, :second_param => second_param}) do
      count = find_letter_count(password_read)

      count >= first_param && count <= second_param
  end

  def letter_position_are_valid(%{:first_param => first_param, :second_param => second_param, :letter => letter, :password => password}) do
      first_criteria = String.at(password, first_param - 1) == letter
      second_critera = String.at(password, second_param - 1) == letter

      (first_criteria && !second_critera) || (second_critera && !first_criteria)
  end

  def is_password_valid(password, "count") do
    letter_count_is_between(read_password(password))
  end

  def is_password_valid(password, "position") do
    letter_position_are_valid(read_password(password))
  end

  def count_valid_passwords(passwords, type) do
    passwords
    |> Enum.filter(fn p -> is_password_valid(p, type) end)
    |> Enum.count()
  end
